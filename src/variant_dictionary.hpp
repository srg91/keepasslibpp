...
namespace keepasslib {
    ...
    private:
        ...
        template<typename T>
        bool get(const std::string &strName, T &t) const;

        template<typename T>
        void set(const std::string &strName, const T &t);

        template<typename T>
        static VdType guess_type(const T t);

        template<typename>
        static VdType guess_type(const VariantValue &vv);

        template<typename>
        static VdType guess_type(const std::string &t);

        template<typename>
        static VdType guess_type(const byte_array_type &t);

...
BOOST_AUTO_TEST_SUITE(test_pw_uuid)
    ...
    BOOST_AUTO_TEST_CASE(test_length) {
        std::string no_throw(Uuid::UuidSize, 0);
        BOOST_CHECK_NO_THROW(Uuid(no_throw).ToString());

        std::string expect_throw(Uuid::UuidSize + 5, 0);
        BOOST_CHECK_THROW(Uuid(expect_throw).ToString(), std::invalid_argument);
    }
    ...
    BOOST_AUTO_TEST_CASE(test_new_uuid) {
        ...
        const size_t default_size = Uuid::UuidSize;
    ...
    BOOST_AUTO_TEST_CASE(test_new_uuid_by_bytes) {
        type::byte_vector expected = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
                                 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f};
        Uuid u(expected);
        auto bytes = u.Bytes();
    ...
    BOOST_AUTO_TEST_CASE(test_lower_operator) {
        ...
        Uuid zero(zero_string), bigger(bigger_string);
    ...
    BOOST_AUTO_TEST_CASE(test_zero) {
        Uuid z = std::string(16, 0);
        BOOST_CHECK_EQUAL(Uuid::Nil, z);
